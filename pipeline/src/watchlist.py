...
#
# 원칙은 "사이트에서 추가·삭제"다(배포 없이 되고, 매집 감시/포지션 관리 구분도
# 화면에서만 가능하다). 예전에 SK하이닉스가 여기 박혀 있다가, 사이트에서 포지션
# 관리로 등록한 뒤에도 이 상수 때문에 파이프라인이 계속 매집 감시로 평가해
# "포지션 관리에서 지우면 매집 감시로 되살아나는 유령"이 됐고, 그래서
# 2026-09-06에 통째로 비웠다.
#
# 아래 12종목은 그 원칙의 의도적인 예외다(2026-09-09). AI 전력 인프라 테마에서
# 실제로 매출·이익이 나오는 쪽을 감시 대상에 올려 매집 구간 진입 여부를 매일
# 판정받기 위한 것으로, 전부 "아직 안 산 종목"이라 매집 감시(accumulation)로만
# 평가되면 된다 — 위의 유령 문제는 같은 종목을 사이트에서 포지션 관리로 등록할
# 때만 생기므로, 이 12종목을 실제로 매수해 포지션 관리에 넣게 되면 그 종목은
# 여기서 지워야 한다.
#
# 사이트 추가 경로(watchlist_tickers)를 안 쓴 이유: `/api/watchlist`에
# MAX_WATCHLIST_SIZE=30 상한이 있는데 테이블에 이미 23종목이 있어 12개가 다
# 안 들어간다. 상한은 파이프라인 실행 시간을 지키려는 안전장치이고, 실측으로
# 감시 종목 평가는 23종목에 약 10초라 35종목이어도 여유가 있다 — 그래서 상한을
# 건드리는 대신 상한이 적용되지 않는 이 상수 쪽에 얹었다.
