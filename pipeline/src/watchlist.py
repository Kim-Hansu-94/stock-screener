...
#
# 2026-09-09에 AI 전력 인프라 12종목을 잠깐 여기 넣었다가 도로 뺐다. 사이트에서
# 지울 수 없는 게 실제로 불편했기 때문이다 — 상한(/api/watchlist의
# MAX_WATCHLIST_SIZE)에 걸려서 상수로 우회했던 건데, 우회 대신 상한을 40으로
# 올리고 종목은 supabase/watchlist_seed_ai_power.sql로 테이블에 넣었다.
# 상한에 걸린다고 이 상수로 도망치지 말 것 — 그 순간 사이트에서 못 지운다.
