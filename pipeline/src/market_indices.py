...
_US_INDEXES = [
    ...
    ("나스닥", "^IXIC"),
    ("S&P500", "^GSPC"),
    # 490590 매수 체크(FOMC 이후 금리 급등 감지)용. **^TNX는 퍼센트 값을 그대로 준다**
    # — 4.96%가 4.961로 저장된다(2026-09-15 db_probe 실측:
    # `미국10년물: close=4.960999965667725`). CBOE 지수 원값은 수익률의 10배지만
    # yfinance가 이미 나눠서 주므로, 화면에서 또 10으로 나누면 안 된다 — 처음에 그렇게
    # 짰다가 뉴스의 "10년물 5.02%"가 화면에 0.50%로 떴다.
    ("미국10년물", "^TNX"),
]
