...
# 거래량 점수의 산 모양 (v6, 2026-09-14). **예전에는 높을수록 만점이었고, 그게 거꾸로였다.**
# 근거는 docstring v6 — 1.0~1.5배가 가장 좋고 1.5배 위로는 무너진다.
VOL_PEAK_LOW = 1.0           # 여기부터 만점 구간 시작
VOL_PEAK_HIGH = 1.5          # 여기까지 만점, 위로는 감점
VOL_FADE_END = 2.0           # 여기서 0점 (1.5~2.0 중간값 2.68%, 2.0배 이상 2.83%)
VOL_FLOOR_SCORE = 0.5        # 하한(0.70)에서 주는 점수 — 1.0 미만도 성적이 나쁘지 않았다
...
def _vol_score_val(vol_ratio: float) -> float:
    """거래량비 → 0~1 점수. **산 모양이다** (v6, 2026-09-14).

    예전 공식은 `min(1, (vol_ratio-1)/2)`로 **높을수록 만점**이었는데, 실측하니
    정확히 거꾸로였다(694건, 60거래일 중간값):

        거래량비        옛 점수     n   중간값    승률   종목수  최다종목%
        1.0배 미만      0점       275   11.06%  61.8%    150      2.2
        1.0~1.5배    0~25점      193   15.93%  62.7%    131      2.1
        1.5~2.0배   25~50점       94    2.68%  51.1%     84      4.3
        2.0배 이상  50~100점      116    2.83%  51.7%     87      3.4

    점수를 가장 많이 받는 구간이 꼴찌였다. 가중치가 0.3이므로 **점수의 30%가 반대
    방향을 가리키고 있었다** — "점수 상위가 오히려 나쁘다"에 대한 설명으로 지금까지
    나온 것 중 가장 잘 들어맞는다.

    그래서 측정된 성과 모양을 그대로 따라가는 사다리꼴로 바꾼다:

        0.70 → 0.5점,  1.0~1.5 → 1.0점(만점),  2.0 이상 → 0점

    세 구간의 순서가 실측 순서(1.0~1.5 > 1.0 미만 > 1.5 초과)와 일치한다.

    ⚠️ 1.0 미만 구간에 기울기를 준 것(0.5 → 1.0)은 **측정된 값이 아니라 가정**이다.
    그 구간을 더 잘게 쪼개 본 적이 없어서 안쪽 모양을 모른다. 다음에 확인할 것.
    """
    if vol_ratio < VOL_PEAK_LOW:
        span = VOL_PEAK_LOW - MIN_VOL_RATIO
        ratio = (vol_ratio - MIN_VOL_RATIO) / span if span > 0 else 1.0
        return VOL_FLOOR_SCORE + (1.0 - VOL_FLOOR_SCORE) * max(0.0, min(1.0, ratio))
