...
v8 (2026-09-15, **거래량 점수를 계단 하나로 — 가정을 걷어낸다**):
  ...
  **하락률 자체가 65~75%에서 한 번 꺼진다.** 60~69점 구간(≈ 하락률 65~75%)이
  ...
  즉 남은 문제는 거래량이나 가중치가 아니라 **하락률 점수의 모양**으로 보였다.
  55~59점 칸도 여전히 마이너스다(-1.54%, 94건 — v7의 -2.26%/118건에서 줄었을 뿐).

조사 (2026-09-15, **하락률 65~75% 골 — 고치지 않기로 했다**):

  v8이 가리킨 "하락률 곡선의 골"을 5% 단위 + 60/120/250거래일로 다시 쟀다
  (run 34923846181, 스윕 10,463건 — 점수·상위선별 없음).

  **① 골은 고정된 구조가 아니다 — 지평선마다 위치가 옮겨 다닌다.**

      구간        60일 중간값   120일    250일
      55~60%        13.77      25.69    60.96
      60~65%        13.40      28.52    67.22
      65~70%        10.84      25.25    51.68  ← 250일 골
      70~75%         7.98      22.02    68.98  ← 60·120일 골
      75~80%        20.30      31.81   111.22

  여기에 점수 모양을 맞추면 **잡음에 맞추는 것**이다. 그래서 채점을 건드리지 않았다.

  **② 대신 승률에서 아주 깨끗한 것이 나왔다.** 중간값보다 훨씬 덜 흔들린다
  (쏠림이 큰 80%+ 두 구간은 뺐다):
  ...
  **정확히 반대 방향이고 둘 다 거의 단조롭다.** 60일에서는 깊을수록 승률이 떨어지고,
  250일에서는 깊을수록 오른다.

  → **깊게 빠진 종목은 나쁜 게 아니라 느리다.** 3개월로는 결판이 안 나고 1년을 봐야 한다.

  **③ 그래서 채점이 아니라 화면·문서로 푼다.** 점수는 보유 기간을 모르고, 보유 기간은
  종목마다 다를 수 있다(사용자 판단). 어느 한쪽 지평선에 점수를 맞추면 다른 쪽을
  쓰는 사람에게 손해다. `DailyReport.tsx`의 안내에 깊이별 승률 표를 넣어
  "70%를 넘으면 1년을 보고 들어가라"를 밝혔다.

  ⚠️ 250일 쪽 숫자에는 **생존 편향이 더 세게** 걸린다(기간이 길수록 망한 종목이
  빠진 효과가 커진다). 다만 **60일에서 깊을수록 승률이 떨어지는 것은 편향으로
  설명되지 않는다** — 편향은 좋게 만들지 나쁘게 만들지 않는다. 그쪽이 더 단단한 관측이다.

  **여전히 모르는 것**: 55~59점 칸이 왜 마이너스인지. 손대지 않았다.
