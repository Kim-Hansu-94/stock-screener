...
v4 (2026-09-14, **백테스트 근거** — `pipeline/research/backtest_pattern_features.py`
    3년 워크포워드 재생, 표본 694건 + 하락률 스윕 10,433건. 한 번 고쳤다가
    재검증에서 나빠져 대부분 되돌렸다. 되돌린 이유까지 남긴다):

  **바꾼 것 — 이평 정배열 보너스 제거 (+0.10 → 없음)**
  정배열이 오히려 나빴다: 60거래일 중간값 5.86%·승률 56.5% vs 정배열 아님
  10.84%·59.3%. 쏠림이 없고(118종목 vs 218종목), 정배열 쪽이 더 유리한
  시기(2025)에 몰렸는데도 졌다. 되살아나면 테스트가 잡는다.

  **그대로 둔 것 — `MIN_DRAWDOWN`(55%)**
  ...
  **되돌린 것 ① — 만점 지점 당기기 (하락률 65%, 소진일수 50일 → 원복)**
  "더 깊게·더 오래 가도 성과가 안 좋아지는데 왜 계속 가산하나"가 근거였는데,
  ...
  **되돌린 것 ② — 저점 높이기 보너스 (+0.10 → 기록만)**
  `_is_higher_low` 주석 참고. 관측에서는 가장 뚜렷했으나 가산점을 주자
  선발 구성이 바뀌며 우위가 사라졌다.

  **넣지 않은 것** — 하락 속도(책과 반대로 급락이 가장 나빴다)와 50% 룰
  (승률 56.6 vs 56.2로 차이 없음).

  **남은 숙제**: "점수 상위가 오히려 나쁘다"는 문제는 만점 지점으로 고쳐지지
  않는다. 점수 구성 요소가 성과와 단조 관계가 아니라서 생기는 문제이고,
  선발 방식 자체를 바꾸는 별도 설계가 필요하다.
...
# 점수 만점 지점. **당겨봤다가 되돌린 값이다** — 근거는 모듈 docstring v4 참고.
# 65%/50일로 당기면 대부분이 만점을 받아 점수가 줄을 못 세운다(70점 이상이 표본의
# 65%가 됐다). 넓게 두는 것이 변별력을 지킨다.
...
def _score_candidate(
    ...
    # 보너스는 VCP 하나뿐이다. 이평 정배열은 성과가 오히려 나빠서 뺐고(docstring v4),
    # 저점 높이기는 가산점으로 주면 선발 구성이 바뀌어 역효과라 기록만 한다.
    vcp_b = _vcp_bonus_val(high, low, close)
    score = min(1.0, base + vcp_b)
