...
**Remaining manual step:** User needs to log into Vercel themselves, import the `stock-screener` GitHub repo, set Root Directory to `frontend`, and add `SUPABASE_URL`/`SUPABASE_SERVICE_KEY` env vars (see `frontend/.env.example`) — this can't be done by the assistant since it needs the user's Vercel login.

**How to apply:** When resuming, check whether the user has completed the Vercel connection (ask, or check if a live URL exists). Next planned work after that: 2단계 (chart-pattern-similarity search) and 3단계 (backtesting), both deliberately deferred per the original design spec.
