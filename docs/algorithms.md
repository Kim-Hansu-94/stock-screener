...
헬스케어·바이오에 그대로 적용하면 오판정이 난다. `frontend/lib/sectorMap.ts`의
`broadSector()` 12개 대분류 중 신뢰할 만한 수치를 찾은 업종만 override했고(임의소비재
1.0/0.6, 유틸리티 1.0/0.6, 산업재 1.2/0.8, 소재 1.5/1.0, 헬스케어 3.0/1.5), 출처마다
범위가 갈려 각 업종 범위의 **가장 낮은(=통과하기 쉬운) 쪽**을 택해 오탐(가짜 취약
판정)을 줄이는 쪽으로 잡았다. 나머지 업종은 기본값(2.0/1.0)을 그대로 쓴다.
