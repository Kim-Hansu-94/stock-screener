...
**복합 점수**(가중합, 상한 1.0):
- 하락률 `0.3 * min(1, (drawdown-0.55)/0.35)` — 90%에서 만점
- 소진일수 `0.4 * min(1, (days-15)/45)` — 60일에서 만점
- 거래량 `0.3 * min(1, max(0, (volRatio-1)/2))`
- 보너스: VCP(`ATR10/ATR50 ≤ 0.6`) **+0.10** — 이것 하나뿐이다

**2026-09-14 백테스트로 손댄 것** (자세한 근거는 `pattern_discovery.py` docstring v4):
- **이평 정배열 보너스를 뺐다** — 정배열이 오히려 나빴다(60거래일 중간값 5.86%·승률
  56.5% vs 정배열 아님 10.84%·59.3%, 쏠림 없음)
- **만점 지점을 65%/50일로 당겼다가 되돌렸다** — 당기니 70점 이상이 표본의 65%가 되어
  점수가 줄을 못 세웠고, 1~5위 성적이 중간값 0.00% → −6.22%로 더 나빠졌다
- **저점 높이기는 기록만 한다** — 관측에서는 가장 뚜렷했지만(중간값 11.72% vs 8.62%)
  +0.10을 주자 해당 종목이 272 → 398건으로 늘며 우위가 사라졌다. **가산점이 선발
  구성을 바꾸기 때문**이다. `recommendation_history.higher_low`로 계속 관측만 한다
- `MIN_DRAWDOWN`(55%)은 스윕으로 검증해 **그대로 뒀다** — 45~55% → 55~65%에서
  중간값이 5.92% → 14.06%로 뛰는 계단이 정확히 거기 있다
...
하락률(55~65 / 65~75 / 75%+) · 순위(1~5 / 6~10 / 11위+) · 점수 구간 · VCP · 저점 높이기 · 거래량 배지 ·
