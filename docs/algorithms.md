...
- 저점 갱신 중단 ≥ `30`거래일(`MIN_DAYS_SINCE_LOW`) — **2026-09-14에 15 → 30으로 올렸다**
...
- 소진일수 `0.4 * min(1, (days-30)/EXHAUSTION_FULL_SPAN)` — 60일에서 만점
...
**2026-09-14 (2) — 소진일수 커트라인 15 → 30일** (v5). v4에서 네 가지가 전부 기각된 뒤,
"점수 상위가 오히려 나쁘다"는 숙제를 **한 번에 하나씩** 푸는 첫 단계다. 15~29일은
694건 중 **유일하게 마이너스인 구간**이었다(중간값 −3.96%·승률 44.0%, 50건, 쏠림 6.0%).
30일을 넘으면 그 뒤로는 12.44 → 12.25 → 10.43으로 거의 평평하므로, 이건 가산점이
아니라 **커트라인**의 성격이다 — v4에서 살아남은 유일한 변경(`MIN_DRAWDOWN` 하드 필터)과
같은 형태다. 만점 지점은 60일 그대로 두려고 `EXHAUSTION_FULL_SPAN`을 45 → 30으로 같이
줄였다(한 번에 두 가지를 바꾸지 않기 위해서다).
