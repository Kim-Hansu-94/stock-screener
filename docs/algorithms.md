...
**복합 점수**(가중합, 상한 1.0):
- 하락률 `0.3 * min(1, (drawdown-0.55)/0.10)` — **65%에서 만점**(2026-09-14 조정)
- 소진일수 `0.4 * min(1, (days-15)/35)` — **50일에서 만점**(2026-09-14 조정)
- 거래량 `0.3 * min(1, max(0, (volRatio-1)/2))`
- 보너스: VCP(`ATR10/ATR50 ≤ 0.6`) **+0.10**, 저점 높이기(최근 20봉 저점 > 직전 20봉 저점) **+0.10**
