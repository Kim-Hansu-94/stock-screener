...
- ~~수급·컨센서스·자사주 소스 미검증~~ — **2026-09-10 프로브로 3/3 확인 완료**
  ...
  - **취득 완료 금액은 이 API에 없다**. 주요사항보고서는 "얼마를 사겠다"는 계획
    ...
    **다만 수집 경로는 아직 못 뚫었다.** KIND는 `searchtotalinfo.do`·`details.do`가
    ...
    → **2026-09-11 결론: 여기서 멈춘다.** 세 가지가 확정됐다.
    (a) **DART에는 없다.** SK하이닉스 최근 30일 공시를 키워드 필터 없이 전부 받아 봤는데
    8/20 취득 시작 이후 일별 신청/체결 공시가 **한 건도 없다**(임원소유상황보고서·
    반기보고서 등뿐). 17영업일간 매일 체결됐으니 DART에 온다면 목록을 뒤덮었을
    것이다 — **일별 체결 내역은 KIND 전용 공시**다.
    (b) **KIND는 `/api/...` 형태를 쓴다**(확장자 없음). 확장자로 경로를 찾던 게
    두 번 연속 실패의 원인이었다 — `.do`만, 다음엔 `.do|.js|.json|.cmd`로 넓혀도
    0건이었는데, 확장자를 아예 버리자 `/api/disclosure/today`·`/api/corpdetails/chart/`
    등 11개가 나왔다. **KRX/KIND 경로를 찾을 때 확장자를 전제하지 말 것.**
    다만 메인 화면 API만 잡혔고 자기주식 화면 API는 그 화면을 열어야 로드된다.
    (c) **브라우저 요청을 복사받는 길도 없다.** 참고한 사이트는 실시간 호출이 아니라
    **저녁 배치 수집**이다(당일 체결량이 18시 이후 공시로 확정되므로) — Network 탭에
    API 호출 자체가 없다.
    → 남은 길은 KIND 화면을 헤드리스 브라우저로 열어 요청을 관찰하는 것뿐인데,
    **KRX는 작업 컨테이너·GitHub Actions 양쪽에서 차단**돼 Playwright를 못 띄운다.
    그래서 ③(기간 경과)까지만 보여주고, 화면이 "이건 달력 이야기일 뿐"이라고
    밝힌다. 다시 시도한다면 KRX에 접근 가능한 환경(로컬 PC 등)에서 요청을
    관찰하는 것부터 시작할 것
