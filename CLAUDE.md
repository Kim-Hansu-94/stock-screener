...
방향으로 가는가를 물으라**는 것이다. **남은 숙제는 하락률 점수의 모양이다** — 지금 점수는
사실상 하락률 순위 그 자체(`0.5 + 0.5 × 하락률점수`)인데, 하락률 곡선이 65~75%에서
한 번 꺼진다(선발 없는 스윕 기준 13.51% → 9.64% → 17.70%). **`_score_candidate`는 일봉 배열만 받는 순수 함수라 과거 날짜로 되돌려 돌릴 수 있다** — 알고리즘 상수를 고칠 근거가 필요하면 실전 성적이 쌓이기를 기다리지 말고 `pipeline/research/backtest_pattern_features.py`로 3년치를 재생할 것 (`docs/backtest-guide.md` 맨 아래 절) |
