...
// 기사는 전부 네이버 뉴스검색에서만 받는다(2026-09-09) — 구글 뉴스는 종목명과
// 무관한 기사가 많이 섞여 들어와서 뺐다.
...
export function StockNewsFeed({ query, className = '' }: { query: string; className?: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // 네이버 호출 자체는 라우트 쪽 revalidate가 막아주므로 부담이 늘지 않는다.
    ...
    <div className={className}>
      <p className="mb-2 text-xs font-semibold text-muted-foreground">
        ...
        <span className="font-normal text-muted-foreground/70">
          {news !== null && news.length > 0 && `· ${SOURCE_LABEL} `}· 1시간마다 갱신
        </span>
      </p>
      {news === null && !failed && (
